...
class ImageVersionShell extends Shell
{
    ...
    public function getOptionParser()
    {
        ...
        $parser->addOption('storageModel', [
            'short' => 's',
            'help' => __d('file_storage', 'The storage model for image processing you want to use.')
        ]);
    ...
    public function initialize()
    {
        ...
        $storageModel = 'Burzum/FileStorage.ImageStorage';
        if (isset($this->params['storageModel'])) {
            $storageModel = $this->params['storageModel'];
        }

        $this->Table = TableRegistry::get($storageModel);

        if (!$this->Table instanceOf ImageStorageTable) {
            $this->out(__d('file_storage', 'Invalid Storage Table: {0}', $storageModel));
